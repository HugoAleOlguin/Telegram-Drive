...
export function DrivePage({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      const uHover = await listen<{ paths: string[] }>('tauri://drag-enter', () => {
        if (active) setIsDragOver(true);
      });
      const uLeave = await listen('tauri://drag-leave', () => {
        if (active) setIsDragOver(false);
      });
      ...
      if (!active) {
        ...
        unlisteners.push(uHover, uLeave, uDrop, uCancel);
      }
    };
    setup();
      ...
      {isDragOver && (
        <div className={styles.dropOverlay}>
          <div className={styles.dropInner}>
            <svg viewBox="0 0 24 24" width="48" height="48" fill="currentColor" stroke="none">
